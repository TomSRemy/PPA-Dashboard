...
def chart_historical_cp(nat_ref, asset_ann, has_asset, asset_name,
    ...
    # ── Bands — single continuous polygon (no discontinuity) ────────────────────
    all_xs = proj_full["year"].tolist()
    fig.add_trace(go.Scatter(
        x=all_xs + all_xs[::-1],
        y=proj_full["p90"].tolist() + proj_full["p10"].tolist()[::-1],
        fill="toself", fillcolor=rgba(ACCENT_WARN, 0.18),
        line=dict(color=transparent()), name="P10-P90",
        showlegend=True, hoverinfo="skip"))
    fig.add_trace(go.Scatter(
        x=all_xs + all_xs[::-1],
        y=proj_full["p75"].tolist() + proj_full["p25"].tolist()[::-1],
    ...
    for i, (_, row) in enumerate(proj_full.iterrows()):
        ...
        # Force last point below; alternate others
        proj_years = proj_full["year"].tolist()
        if row["year"] == proj_years[-1]:
            ay_val = 48
        else:
            _idx = proj_years.index(row["year"])
            ay_val = -42 if _idx % 2 == 0 else 42
        fig.add_annotation(
