"""
PPA Pricing Dashboard v2.5
Modular architecture. Logic in compute.py | charts.py | data.py | ui.py | config.py | excel.py
Changes v2.5:
- Tab8 Market Prices restructured (5 sections: last 7d, monthly history, spreads, balancing, KPIs)
- Tab5 waterfall imports updated to match new chart_waterfall signature
- chart_da_history / chart_imbalance_history / chart_price_comparison / chart_imbalance_spread
  replaced by chart_last_week / chart_da_monthly / chart_intraday_spread / chart_imbalance_vs_da
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# PPA PREMIUMS — session_state (read before compute)
# ══════════════════════════════════════════════════════════════════════════════
...
# Scatter + proj_targets — computed before tabs
...
with tab5:
    ...
    # Re-read updated values from session_state after widgets
    imb_eur        = float(st.session_state.imb_eur)
    add_disc       = float(st.session_state.add_disc) / 100
    vol_risk_pct   = float(st.session_state.vol_risk_pct) / 100
    price_risk_pct = float(st.session_state.price_risk_pct) / 100
...
st.markdown(
    ...
    f'v2.5 — ENTSO-E France {data_start.year}–{data_end.strftime("%Y-%m-%d")} '
