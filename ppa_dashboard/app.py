...
with st.sidebar:
    ...
    st.markdown(
        f'<div class="update-pill">{load_log().split(chr(10))[0]}</div>',
        unsafe_allow_html=True
    )
    st.markdown("---")

    st.markdown("### Technology")
    techno = st.selectbox("Select technology", ["Solar", "Wind"], index=0, label_visibility="collapsed")
    ...
    tenor_start = st.number_input(
        "Tenor start (year)",
        min_value=_last_yr + 1,
        max_value=_last_yr + 20,
        value=_last_yr + 1,
        step=1,
        key="tenor_start"
    )
    tenor_end = st.number_input(
        "Tenor end (year)",
        min_value=_last_yr + 1,
        max_value=_last_yr + 20,
        value=_last_yr + 5,
    ...
    st.download_button(
        ...
        file_name="example_load_curve.csv",
        mime="text/csv",
        key="dl_example"
    )
...
# ══════════════════════════════════════════════════════════════════════════════
if "imb_eur" not in st.session_state:
    st.session_state.imb_eur = 1.9
if "add_disc" not in st.session_state:
    st.session_state.add_disc = 0.0
if "vol_risk_pct" not in st.session_state:
    st.session_state.vol_risk_pct = 0.0
if "price_risk_pct" not in st.session_state:
    st.session_state.price_risk_pct = 0.0
if "goo_value" not in st.session_state:
    st.session_state.goo_value = 1.0
...
fwd_df = pd.DataFrame(
    [{"year": yr, "forward": float(DEFAULT_FWD.get(yr, 52.0))} for yr in range(tenor_start, tenor_end + 1)]
)
...
with tab1:
    ...
    with ca:
        ytd_note = (
            f" — <span class='ytd-badge'>2026 YTD included</span>"
    ...
    if techno == "Wind" and not has_wind:
        status_msg(
            "Wind data (WindMW) not yet in hourly_spot.csv — "
            "run the updated ENTSO-E script (B18+B19). Solar shown as fallback.",
            kind="wind"
        )
    else:
    ...
    with k4:
        p50_pnl = (
            vol_mwh * (ref_fwd * (1 - float(np.percentile(hist_sd_f, 50))) - ppa) / 1000
...
with tab3:
    ...
    st.plotly_chart(
        chart_shape_disc_delta(nat_ref, cfg["nat_sd"], cfg["color"], cfg["label"]),
        use_container_width=True
    )
    ...
    st.plotly_chart(
        chart_heatmap(monthly_agg, cfg["color"], cfg["label"]),
        use_container_width=True
    )

    st.markdown("---")
    st.markdown(
        '<div class="section-title">Market Value Analysis — Jomaux / GEM Energy Analytics</div>',
        unsafe_allow_html=True
    )
    section(f"Market Value vs {cfg['label']} Generation Output")
    desc(
        f"Average day-ahead price per MW bin. Cannibalization mechanism. "
        f"Method: GEM Energy Analytics 'The decreasing market value of renewables' (Oct 2024)."
    )
    st.plotly_chart(
        chart_market_value_vs_penetration(
            hourly, cfg["prod_col"], cfg["color"], cfg["label"], partial_years
        ),
        use_container_width=True
    )
    ...
    with j1:
        ...
        desc(
            "Normalised day-ahead prices by hour of day, one line per year. "
    ...
    with j2:
        ...
        st.plotly_chart(
            chart_canyon_curve(
                hourly, cfg["color"], cfg["label"], cfg["duck_months"], recent_years=4
            ),
            use_container_width=True
        )
...
with tab5:
    ...
    w1, w2 = st.columns(2)
    with w1:
        st.number_input("Imbalance Cost (EUR/MWh)", 0.0, 10.0, step=0.1, key="imb_eur", help="Suggested: 1.9 EUR/MWh")
        st.number_input("Volume Risk (%)", 0.0, 10.0, step=0.1, key="vol_risk_pct", help="Suggested: 2.5%")
        st.number_input("Price Risk (%)", 0.0, 10.0, step=0.1, key="price_risk_pct", help="Suggested: 3.0%")
    with w2:
        st.slider("Additional Discount (%)", 0.0, 10.0, step=0.25, key="add_disc")
        st.number_input("GoO Value (EUR/MWh)", 0.0, 10.0, step=0.1, key="goo_value", help="Suggested: 1.0 EUR/MWh")
        st.number_input("Margin (EUR/MWh)", 0.0, 10.0, step=0.1, key="margin", help="Suggested: 1.0 EUR/MWh")
    ...
    def _style_params(row):
        ...
        if row["Type"] == "Addition":
            return [f"background-color:{C2L}"] * len(row)
        if row["Type"] == "Total":
            return [f"background-color:{C1};color:white;font-weight:bold"] * len(row)
        return [""] * len(row)

    styled = params_df.style.apply(_style_params, axis=1)
    st.dataframe(styled, use_container_width=True, hide_index=True, column_order=["Component", "Value (EUR/MWh)"])

    st.plotly_chart(
        chart_waterfall(
            ref_fwd, sd_ch, imb_eur, cfg["label"],
            vol_risk_pct=vol_risk_pct,
            price_risk_pct=price_risk_pct,
            cannib_risk_pct=0.0,
            goo_value=goo_value,
...
with tab6:
    ...
    if techno == "Wind" and not has_wind:
        status_msg(
            "Wind data not yet available — run the ENTSO-E update script. "
            "Solar rolling M0 shown as fallback.",
            kind="wind"
        )
    ...
    if roll is None or len(roll) < 10:
        ...
        st.plotly_chart(
            chart_rolling_cp(
                roll, nat_ref_complete, nat_ref, cfg["nat_cp"],
        ...
        if sum_rows:
            ...
            st.dataframe(
                pd.DataFrame(sum_rows).style.apply(_hi_sum, axis=1),
                use_container_width=True,
                hide_index=True
            )
...
with tab7:
    ...
    if uploaded_conv:
        try:
            df_conv = pd.read_csv(uploaded_conv) if uploaded_conv.name.endswith(".csv") else pd.read_excel(uploaded_conv)
                ...
                try:
                    ...
                    if incl_solar:
                        try:
                            ...
                            st.warning(f"Solar unavailable: {e2}")

                    if incl_wind:
                        try:
                            on = _fetch_gen("B19")
                            on.index = on.index.tz_localize(None)
                            off = _fetch_gen("B18")
                            ...
                            st.warning(f"Wind unavailable: {e3}")
                    ...
                    st.error("entsoe-py not installed.")
                except Exception as e:
...
with tab8:
    ...
    if bal is None or len(bal) == 0:
        ...
        status_msg(
            f"Balancing data: {bal_start.strftime('%Y-%m-%d')} "
        ...
        desc("Average DA price by hour and month across all years. Shows intraday and seasonal price patterns.")
        st.plotly_chart(chart_da_heatmap(bal), use_container_width=True)

        st.markdown("---")
        section("DA vs Imbalance Prices")
        desc("Monthly average DA price vs imbalance positive/negative prices. Large spread = high balancing costs for generators.")
        st.plotly_chart(chart_price_comparison(bal), use_container_width=True)

        st.markdown("---")
        section("Imbalance Spread — Positive minus Negative")
        desc("High spread = high cost of being short vs long. Relevant for imbalance risk assessment in PPA pricing.")
        st.plotly_chart(chart_imbalance_spread(bal), use_container_width=True)

        st.markdown("---")
        section("Balancing Services — aFRR & mFRR")
        desc(
            "Monthly average activated balancing energy prices. "
